...
class PhoneListenerService : WearableListenerService() {
    ...
    private val START_ACTIVITY = "/start_phone_activity"
    override fun onMessageReceived(messageEvent: MessageEvent?) {
        println("!!! $START_ACTIVITY")
        if (messageEvent?.path.equals(
                START_ACTIVITY,
                ignoreCase = true
            )
        ) {
...
//                if (path == PhoneActivity.WEARABLE_DATA_PATH) {
//                }
...
//        if (messageEvent.path == PhoneActivity.WEARABLE_DATA_PATH) {
